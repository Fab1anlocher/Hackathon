...
# Find which features have high correlations
high_cor_mask <- apply(abs(cor_matrix) > 0.7, 1, any) & 
                 apply(abs(cor_matrix) > 0.7, 2, any)
# Remove diagonal (self-correlation)
diag_mask <- diag(cor_matrix) == 1
high_cor_mask <- high_cor_mask & !diag_mask

# Get features with meaningful correlations
features_to_show <- rownames(cor_matrix)[apply(abs(cor_matrix) > 0.7, 1, function(x) sum(x) > 1)]

if(length(features_to_show) > 50) {
  # If still too many, show only the top 30 most connected features
  cor_connections <- apply(abs(cor_matrix) > 0.7, 1, sum)
  top_features <- names(sort(cor_connections, decreasing = TRUE)[1:30])
...
# Convert to long format and find top correlations
cor_long <- data.frame(
  Feature1 = rep(rownames(cor_upper), ncol(cor_upper)),
  Feature2 = rep(colnames(cor_upper), each = nrow(cor_upper)),
  Correlation = as.vector(cor_upper)
)
cor_long <- cor_long[!is.na(cor_long$Correlation), ]
cor_long <- cor_long[order(-abs(cor_long$Correlation)), ]
cor_long$Abs_Correlation <- abs(cor_long$Correlation)

if(nrow(cor_long) > 0) {
  top_correlations <- head(cor_long[, c("Feature1", "Feature2", "Correlation")], 10)
...
if(length(factor_col_names) > 0) {
  ...
  # For age_group, convert to numeric (ordinal encoding makes sense for age categories)
  if("age_group" %in% names(train_data)) {
    # Create numeric version of age_group for training
    train_data$age_group_numeric <- as.numeric(train_data$age_group)
    test_data$age_group_numeric <- as.numeric(test_data$age_group)
    
    # Update feature list to use numeric version
    feature_cols_rf <- setdiff(names(train_data), c("default", "age_group"))
  }
  
  # Handle any other factor columns if present
  for(col in factor_col_names) {
    if(col != "age_group" && col %in% names(train_data)) {
