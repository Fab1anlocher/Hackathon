...
ggplot(bondora_clean_dti, aes(x = factor(default), y = DebtToIncome, fill = factor(default))) +
  geom_boxplot() +
  coord_cartesian(ylim = c(0, quantile(bondora_clean_dti$DebtToIncome, 0.95, na.rm = TRUE))) +
  labs(title = "Debt-to-Income Ratio by Default Status",
       subtitle = "(Outliers above 95th percentile excluded for visibility)",
       x = "Default Status (0 = Non-Default, 1 = Default)",
...
p2 <- ggplot(bondora, aes(x = factor(default), y = AmountOfPreviousLoansBeforeLoan, fill = factor(default))) +
  geom_boxplot() +
  coord_cartesian(ylim = c(0, quantile(bondora$AmountOfPreviousLoansBeforeLoan, 0.90, na.rm = TRUE))) +
...
feature_cols <- setdiff(names(train_data), c("default", "age_group"))  # Exclude categorical for logistic

# For logistic regression, use only numeric features
...
- **ntree = 100:** Using 100 trees provides stable predictions without excessive computational cost.
