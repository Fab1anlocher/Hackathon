...
2. **Price-Risk Relationship (Interest Rate):** The strong correlation between interest rates and default outcomes confirms that Bondora's pricing mechanism successfully identifies riskier borrowers. However, this also means interest rates can serve as a powerful predictive feature in our model.
...
### Discussion: Do We Need Data Transformations?

**Yes, transformations are essential** for this dataset due to several data quality and modeling challenges identified during EDA:

**1. Missing Value Treatment:**

*Problem Identified:* During exploration, we found missing values in several financial variables (DebtToIncome, PreviousLoans, etc.).
...
- **Alternative considered:** Creating missing indicators as binary features could capture the "missingness as signal" but adds complexity. We prioritize simplicity for this first model iteration
...
- **Maintains interpretability:** By keeping one feature from each correlated pair, we retain the information while improving coefficient interpretability
...
We applied the following transformations to prepare the Bondora dataset for modeling:
...
# We need to exclude factor variables like age_group or convert them to numeric
...
cat("\nTraining Random Forest...\n",
    "Note: This training process is expected to take 2-5 minutes due to:\n",
    "  - Cross-validation with SMOTE resampling on each fold\n",
    "  - Large dataset size and multiple hyperparameter combinations\n",
    "  - This duration is normal and ensures robust model performance\n\n", sep = "")
...
*Business implication:* We CANNOT use accuracy as our primary metric for loan approval decisions. A loan officer using accuracy would be misled about the model's true value.

### 2. **Sensitivity/Recall** (`r round(cm_rf$byClass["Sensitivity"], 4)`): The Default Detection Rate

*What it measures:* Of all loans that actually defaulted, what percentage did we correctly flag as high-risk?
...
*Current performance:* We catch `r round(cm_rf$byClass["Sensitivity"] * 100, 1)`% of defaults, meaning we miss `r round((1 - cm_rf$byClass["Sensitivity"]) * 100, 1)`% of risky loans.

*Business implication:* If Bondora processes 1,000 loan applications with 100 expected defaults (10% rate), this sensitivity means we would correctly reject `r round(cm_rf$byClass["Sensitivity"] * 100, 0)` of those risky loans but still approve `r round((1 - cm_rf$byClass["Sensitivity"]) * 100, 0)` bad loans.
...
*What it measures:* Of all loans that would NOT default, what percentage did we correctly approve?
...
*Current performance:* We correctly approve `r round(cm_rf$byClass["Specificity"] * 100, 1)`% of good loans, rejecting only `r round((1 - cm_rf$byClass["Specificity"]) * 100, 1)`% of creditworthy applications.
...
*What it measures:* When our model flags a loan as "will default," how often is that prediction correct?

*Why it matters for Bondora:* Low precision means many false alarms—rejecting borrowers who would have repaid. This metric tells us how much to trust a "default" prediction.

*Current performance:* `r round(cm_rf$byClass["Precision"] * 100, 1)`% of loans we reject would have actually defaulted, meaning `r round((1 - cm_rf$byClass["Precision"]) * 100, 1)`% of rejections are good borrowers we turned away unnecessarily.
