...
Um dieses R Markdown Dokument zu rendern, haben Sie drei Möglichkeiten:

1. **In RStudio:** Klicken Sie einfach auf den "Knit" Button oben im Editor
2. **In der R Console:** Führen Sie aus: `rmarkdown::render("Bondora_Classification_Analysis.Rmd")`
3. **Via Script:** Führen Sie aus: `Rscript render_notebook.R`

**Hinweise:**
- Beim ersten Mal werden fehlende Pakete (insbesondere `themis`) automatisch installiert
- Der Random Forest Trainingsvorgang dauert ca. 2-5 Minuten - das ist normal und gewährleistet robuste Modellleistung
- Stellen Sie sicher, dass `Bondora.csv` und `Description.csv` im selben Verzeichnis sind

**English:**

To render this R Markdown document, you have three options:

1. **In RStudio:** Simply click the "Knit" button at the top of the editor
2. **In R Console:** Run: `rmarkdown::render("Bondora_Classification_Analysis.Rmd")`
3. **Via Script:** Run: `Rscript render_notebook.R`

**Notes:**
- Missing packages (especially `themis`) will be automatically installed on first run
- Random Forest training takes approximately 2-5 minutes - this is normal and ensures robust model performance
- Ensure `Bondora.csv` and `Description.csv` are in the same directory
